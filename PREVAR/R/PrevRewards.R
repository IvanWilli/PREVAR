...
PrevVar2 <- function(
		...
		type = 2,      # 
		interval = 1,  # otherwise it's prevalence
		rewards = "Bernoulli"){ # "bernoulli" different
	# just a question of where to put it if the time interval is e.g. .5
	s       <- length(qx) # nr of ages
	s1      <- s + 1
	stopifnot(s == length(prevx))
	...
	if(type == 1){
		...
		if (rewards == "fixed"){
			# fixed means uniform
			m2 <- (prevx * interval)^ 2  # put interval inside?
			m3 <- (prevx * interval)^ 3 
		} else {       
			# Bernoulli outcome
			m2 <- prevx * interval
...
VarBernoulli <- function(qx,prev,interval, calcs = "orig",closeout=FALSE){
	# just a question of where to put it if the time interval is e.g. .5
	s       <- length(qx) # nr of ages
	s1      <- s + 1
	...
	# from paper
	R <- matrix(m,nrow=s1,ncol=s,byrow=TRUE)
	R <- cbind(R,0)
	if (closeout){
		R[s1, ] <- R[s1, ] / 2
	}
	...
	if (calcs == "faster"){
		...
		rho1 <- colSums( N * colSums(P * R)[-s1])
		rho2 <- colSums(N *
						(
							colSums(P * R)[-s1] +
							colSums(2 *(P*R)[-s1,-s1] * c(rho1))
							))
	}
# original Caswell code:
	if (calcs == "orig"){
...
#					)
		# from class
		rho1 <- NN%*%(Z%*%((t(P*R))%*%e))
		rho2 <- NN%*%(Z%*%((t(P*R))%*%e)+2*Z%*%(t(P*R))%*%t(Z)%*%rho1)
...
#rho2_target <- VDLE + DLE^2
#rho2 ^ 2
#rho2*2
#surho2m(NN[1,] * (Z%*%((t(P*R))%*%e)+2*Z%*%(t(P*R))%*%t(Z)%*%rho1))
#
#rightside <- 
#			colSums(P * R)[-s1] +
#			colSums(2 *(P*R)[-s1,-s1] * c(rho1))
