...
\address[id=aff2]{%
  \orgname{Universidad Nacional de Luján}
...
In the following we first clarify the data constraints under which the state variance problem is posed. We then illustrate the two variance methods that have thus far been proposed using a Markov rewards approach. We then describe a discrete perspective on this problem based on survivor quantile intervals and age intervals. Within this discrete framework, we find the minimum and maximum, and other schematic variances that are possible under a given set of Sullivan inputs. We then empirically motivate and propose a new indirect method to estimate state variance under Sullivan constraints, and then compare the results of this method with the variance estimate of a comparable multistate model.
...
%\end{equation}
%where in practice the outer expectation is taken as the sum of the squared residuals duly weighted by their respective probabilities of occurring. The second is
...
% given a fixed mean, minimize the variance is equal to minimize second moment
...
\section*{Acknowledgements}
   Thanks to \censor{Douglas Wolf} and \censor{Jennifer Karas Montez} for posing a question that led to this work, to \censor{Hal Caswell} and \censor{Virginia Zarulli} for the groundwork on which this is based and for fielding inquiries, and to \censor{Iñaki Permanyer} for helpful discussions..
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%                  The Bibliography                       %%
%%                                                         %%
